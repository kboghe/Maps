...
def scrape_pop(driver,search_input):
    ...
    if popular_times_available > 0:  # if you have found the popular times graph (length of elements found is greater than zero).
        ...
        for i in range(3):
            try:
                day_of_week = driver.find_element_by_css_selector('div.goog-inline-block.goog-menu-button-caption')
                break
            except NoSuchElementException:
                time.sleep(5)
                continue
        ...
        try:
            ...
            print("Skipped this place due to weird UnboundLocalError!")
            popular_times_available = 0
